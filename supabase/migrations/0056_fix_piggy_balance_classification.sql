-- 0056_fix_piggy_balance_classification.sql
-- Keeps Piggy Savings aligned with confirmed Piggy wallet ledger rows and keeps pending payments separate.
...
as $$
  ...
  normalized_payments as (
    select
      pt.*,
      case
        when lower(replace(coalesce(pt.payment_type::text, ''), ' ', '_')) in ('piggy_bag', 'piggy', 'piggy_box', 'piggybag') then 'piggy_bag'
        when lower(replace(coalesce(pt.payment_type::text, ''), ' ', '_')) in ('savings', 'saving', 'savings_plan', 'personal_susu', 'personal_susu_deposit') then 'savings'
        when lower(replace(coalesce(pt.payment_type::text, ''), ' ', '_')) in ('wallet_deposit', 'deposit') then 'wallet_deposit'
        when lower(replace(coalesce(pt.payment_type::text, ''), ' ', '_')) in ('contribution', 'susu_contribution', 'circle_contribution') then 'contribution'
  ...
  select
    ...
    coalesce((select sum(amount) from confirmed_wallet where transaction_type = 'contribution_payment'), 0),
    ...
    coalesce((select sum(amount) from confirmed_wallet where transaction_type in ('savings_deposit', 'personal_susu_deposit')), 0),
    coalesce((select sum(amount) from confirmed_wallet where transaction_type = 'piggy_bag_deposit'), 0),
    coalesce((select sum(amount) from confirmed_wallet where transaction_type = 'deposit'), 0),
...
as $$
  ...
  confirmed_wallet as (
    select
      case
        when wt.transaction_type = 'piggy_bag_deposit' then 'piggy_bag'
        when wt.transaction_type in ('savings_deposit', 'personal_susu_deposit') then 'savings'
        when wt.transaction_type = 'contribution_payment' then 'contribution'
        when wt.transaction_type = 'deposit' then 'wallet_deposit'
...
as $$
  with confirmed_plan_payments as (
    select
      ...
      wt.created_at
    from public.wallet_transactions wt
    join public.payment_transactions pt on pt.id = wt.payment_transaction_id
    where wt.user_id = auth.uid()
      and wt.status in ('successful', 'confirmed')
      and wt.transaction_type = 'piggy_bag_deposit'
  ),
  paid_deposits as (
    select
      deposit.plan_id,
  ...
  plan_money as (
    ...
    where not exists (
      select 1
      from public.wallet_transactions wt
      where wt.payment_transaction_id = pd.payment_transaction_id
        and wt.status in ('successful', 'confirmed')
        and wt.transaction_type = 'piggy_bag_deposit'
    )
  )
