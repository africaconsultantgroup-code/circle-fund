...
import { circles, currentUser, formatGHS, notifications, verification, verificationProgress, isFullyVerified, trustScore, riskAlerts } from "@/lib/mock-data";
...
function HomePage() {
  ...
  const totalMembers = circles.reduce((a, c) => a + c.members.length, 0);
    ...
    <div className="flex flex-col">
      ...
      <div className="-mt-16 px-5">
        <div className="grid grid-cols-3 gap-3 rounded-3xl bg-card p-4 shadow-elevated">
          ...
          <Stat label="Cycles" value={8} icon={<ArrowUpRight className="h-4 w-4" />} />
        </div>
      </div>
      ...
      <section className="mt-7 px-5">
        ...
        <div className="mt-3 rounded-3xl border border-border bg-card p-4 shadow-card">
          ...
          <p className="mt-2 font-display text-2xl font-bold">{formatGHS(circles[1].amount * circles[1].members.length)}</p>
          <p className="text-xs text-muted-foreground">
            To {circles[1].nextRecipient} · {circles[1].nextPayoutDate}
          </p>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-muted">
            <div
              className="h-full rounded-full bg-gradient-primary"
              style={{ width: `${(circles[1].currentCycle / circles[1].totalCycles) * 100}%` }}
            />
          </div>
          <p className="mt-2 text-[11px] text-muted-foreground">
            Cycle {circles[1].currentCycle} of {circles[1].totalCycles}
          </p>
        </div>
      </section>
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {c.members.length} members · {formatGHS(c.amount)}/{c.frequency}
                  </p>
