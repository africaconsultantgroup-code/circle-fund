...
export function HomePage() {
  ...
  const totalFinancialPosition =
    dashboard.availableWalletBalance + dashboard.lockedBalance + dashboard.totalContributed;
  const unread = dashboard.notifications.length;
  const activeParticipationCircles = dashboard.circles.filter((circle) =>
    ["pending", "pending_capacity_review", "approved"].includes(circle.membershipStatus),
  ).length;
  ...
  const loadDashboard = useCallback(async () => {
    setIsLoading(true);
    const [data, gateResult, createResult] = await Promise.all([
  ...
  const handleFundSavingsPlan = async () => {
    const amount = Number(
      nextContribution?.amount_due ??
        nextContribution?.amount ??
        dashboard.circles[0]?.amount ??
        100,
    );
    setPaymentError("");
  ...
  return (
    <div className="flex flex-col pb-8">
      <header className="bg-gradient-card px-5 pb-24 pt-12 text-primary-foreground">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-primary-foreground/65">
              Your SikaCircle financial dashboard
            </p>
          </div>
          <Link
            to="/notifications"
        ...
        <div className="mt-8">
          ...
          <p className="mt-1 font-display text-4xl font-bold tracking-tight">
            {formatCurrency(totalFinancialPosition, primaryCurrency)}
          </p>
          <p className="mt-2 text-xs text-primary-foreground/70">
            Confirmed wallet, contributions, Piggy Box, and savings balances.
          </p>
        </div>
      </header>
      ...
      <div className="-mt-16 px-5">
        <div className="grid grid-cols-2 gap-3 rounded-3xl bg-card p-4 shadow-elevated">
          <SummaryTile
            ...
            value={String(dashboard.circles.length)}
          />
          <SummaryTile
            icon={<CalendarDays className="h-4 w-4" />}
            label="Upcoming"
            value={String(dashboard.upcomingContributions.length)}
          />
          <SummaryTile
            icon={<ArrowUpRight className="h-4 w-4" />}
            label="Contributed"
            value={formatCurrency(dashboard.totalContributed, primaryCurrency)}
          />
          <SummaryTile
            icon={<ArrowDownLeft className="h-4 w-4" />}
            label="Received"
            value={formatCurrency(dashboard.totalReceived, primaryCurrency)}
          />
        </div>
      ...
      <section className="mt-5 px-5">
        <div className="rounded-2xl border border-border bg-card p-4 shadow-card">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <BreakdownMetric
              label="Total Paid"
              value={formatCurrency(dashboard.totalPaid, primaryCurrency)}
            />
            <BreakdownMetric
              label="Susu Contributions"
              value={formatCurrency(dashboard.susuContributions, primaryCurrency)}
            />
            <BreakdownMetric
              label="Piggy Savings"
              value={formatCurrency(dashboard.piggySavings, "GHS")}
            />
            <BreakdownMetric
              label="Wallet Deposits"
              value={formatCurrency(dashboard.walletDeposits, primaryCurrency)}
            />
            <BreakdownMetric
            ...
            <BreakdownMetric
              ...
              value={formatCurrency(dashboard.expectedPayoutTotal, primaryCurrency)}
            />
            <BreakdownMetric
              label="Pending Payments"
              value={formatCurrency(dashboard.pendingPayments, primaryCurrency)}
            />
            <BreakdownMetric
              label="Wallet available"
              value={formatCurrency(dashboard.availableWalletBalance, primaryCurrency)}
            />
            <BreakdownMetric
              label="Locked balance"
              value={formatCurrency(dashboard.lockedBalance, primaryCurrency)}
            />
          </div>
        </div>
      </section>
          ...
          <>
            ...
            >
              ...
              <div>
                ...
                <p
                  className={`text-[11px] ${creatorLimitReached ? "text-muted-foreground" : "text-primary-foreground/70"}`}
                >
                  {creatorLimitReached ? "2 admin limit reached" : "New circle"}
                </p>
              </div>
            </Link>
            <Link
          ...
          <div className="rounded-2xl border border-gold/40 bg-gold/10 p-4 text-[color:var(--gold-foreground)]">
            ...
            {creatorLimitReached && (
              <p className="mt-1 text-xs">
                {createLimit?.message ?? "You can only administer 2 active susu groups at a time."}
              </p>
            )}
            {participationReviewNeeded && (
              <p className="mt-1 text-xs">
                You are already in 3 active susu groups. SikaCircle must review your capacity before
                approving another group.
              </p>
            )}
          </div>
        </section>
      ...
      <section className="mt-7 px-5">
        ...
        {dashboard.nextPayout ? (
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-display text-base font-semibold">
                  {dashboard.nextPayout.circle_name ?? "Susu payout"}
                </p>
          ...
          <EmptyState
            ...
            text="No payout date is scheduled yet. Your payout turn will appear after a circle rotation is generated."
          />
        )}
      </section>
            ...
            {dashboard.upcomingContributions.slice(0, 3).map((contribution) => {
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        ...
                        {formatContributionStatus(contribution.status)}
                      </p>
                    </div>
                    <p className="font-display text-sm font-semibold">
                      {formatCurrency(
                  ...
                  >
                    {busyPayment === `wallet-${contribution.id}` ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
                      <Wallet className="h-4 w-4" />
                    )}
      ...
      <SavingsPlanner
        ...
        defaultDueDate={toDateInputValue(
          nextContribution?.due_date ?? dashboard.circles[0]?.nextPayoutDate,
        )}
        currency={primaryCurrency}
      />
      ...
      <section className="mt-7 px-5">
        ...
        {dashboard.circles.length > 0 ? (
              ...
              <li key={circle.id}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <CircleProgressDetails
                      circle={circle}
                      financial={dashboard.circleFinancials[circle.id]}
                    />
                    <p className="text-[11px] text-muted-foreground">
                      {circle.memberCount}/{circle.maxMembers} members ·{" "}
                      {formatCurrency(circle.amount, circle.baseCurrency)}/{circle.frequency}
                    </p>
                  </div>
          ...
          <EmptyState
            icon={<Users className="h-4 w-4" />}
            text="You have no active circles yet. Create or join one to get started."
          />
        )}
      </section>
...
export async function loadFinancialDashboard(): Promise<DashboardData> {
  const [profile, user, circleResult] = await Promise.all([
  ...
  const [
    ...
    supabase.from("contributions").select("circle_id, status").eq("user_id", user.id),
    supabase.from("payouts").select("*").eq("user_id", user.id).eq("status", "completed"),
    getCustomerFinancialSummary(),
  ...
  const totalReceived =
    ...
    ((payoutsResult.data ?? []) as Payout[]).reduce(
  ...
  return {
    ...
    circleFinancials: buildCircleFinancials(
      circleResult.data,
      allContributionsResult.data ?? [],
      payoutTurnsResult.data,
    ),
    notifications: buildNotifications({
      ...
      piggyBoxBalance: Number(summary.piggy_balance ?? 0),
      participationReviewNeeded:
        circleResult.data.filter((circle) =>
          ["pending", "approved"].includes(circle.membershipStatus),
        ).length >= 3,
    }),
...
async function loadNextPayoutTurn(userId: string) {
  const { data, error } = await supabase
    ...
    .select(
      "id, circle_id, payout_due_date, payout_amount, status, circles(name), circle_members!inner(user_id)",
    )
...
async function loadPayoutTurns(userId: string) {
  const { data, error } = await supabase
    .from("payout_schedule")
    .select(
      "id, circle_id, rotation_position, payout_due_date, payout_amount, status, circle_members!inner(user_id)",
    )
    .eq("circle_members.user_id", userId);

  if (error)
    return { data: [] as Array<PayoutTurn & { rotation_position: number }>, error: error.message };
...
function buildCircleFinancials(
  ...
  return circles.reduce<Record<string, CircleFinancialSummary>>((acc, circle) => {
    ...
    const paidContributions = contributions.filter((row) =>
      ["paid", "processed"].includes(row.status),
    ).length;
...
function buildNotifications({
  ...
  const dueSoon = upcomingContributions.find(
    (contribution) => daysUntil(contribution.due_date) <= 7,
  );
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-card">
      <div className="flex items-center gap-3">
        <span
          className={`flex h-10 w-10 items-center justify-center rounded-xl ${complete ? "bg-success/10 text-success" : "bg-gold/20 text-[color:var(--gold-foreground)]"}`}
        >
        ...
        <div className="flex-1">
          ...
          {complete && (
            <p className="text-[11px] text-muted-foreground">
              Verification complete. Circle actions are unlocked.
            </p>
          )}
          {!complete && formsComplete && (
            <p className="text-[11px] text-muted-foreground">
              Verification forms complete. Review is pending.
            </p>
          )}
          {!complete && !formsComplete && (
            <p className="text-[11px] text-muted-foreground">
              Continue verification before money movement starts.
            </p>
          )}
        </div>
        <Link
          to={
            complete || formsComplete ? "/verify/status" : (gateSummary?.nextStep.to ?? "/verify")
          }
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <StatusLine
          label="Phone"
          value={statusLabel(statuses?.phone)}
          good={statuses?.phone === "verified"}
        />
        <StatusLine
          label="KYC"
          value={statusLabel(statuses?.ghanaCard)}
          good={statusAccepted(statuses?.ghanaCard)}
        />
      </div>
    </div>
  );
...
function CircleProgressDetails({
  circle,
  financial,
}: {
  circle: UserCircle;
  financial?: CircleFinancialSummary;
}) {
  ...
  return (
    <div className="mt-2">
      <div className="grid grid-cols-2 gap-1.5">
        <MiniMetric
          label="My Position"
          value={financial?.myPosition ? `#${financial.myPosition}` : "Not set"}
        />
        <MiniMetric
          label="Expected Payout"
          value={formatDate(financial?.expectedPayoutDate ?? circle.nextPayoutDate)}
        />
...
function PriorityPill({ priority }: { priority: NotificationItem["priority"] }) {
  ...
  return (
    <span
      className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${classes}`}
    >
      {priority}
    </span>
  );
...
function StatusLine({ label, value, good }: { label: string; value: string; good: boolean }) {
  return (
    <div className="rounded-xl bg-muted/40 px-3 py-2">
      <p className="text-[10px] uppercase tracking-wide text-muted-foreground">{label}</p>
      <p
        className={`mt-0.5 text-xs font-semibold ${good ? "text-success" : "text-muted-foreground"}`}
      >
        {value}
      </p>
    </div>
...
function DisabledAction({ icon, title }: { icon: ReactNode; title: string }) {
  return (
    <button
      disabled
      className="flex items-center gap-3 rounded-2xl border border-border bg-card p-4 text-left opacity-50 shadow-card"
    >
      <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary text-primary">
        {icon}
      </span>
